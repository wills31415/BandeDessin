...
PNMImage* packComic(const PNMImage** images, size_t nbImages, size_t comicWidth, size_t comicBorder, size_t comicInterval)
{
    ...
    /* Initialisation de la variable "shared", partagée entre les threads. */
    shared_arg_t shared;
    shared.nb_images_done = (size_t*)malloc(sizeof(size_t));
    *(shared.nb_images_done) = 0;
    ...
    for(size_t i = 0; i < nbImages; ++i) arg[i] = (arg_t){images[i], cost[i], shared};
    ...
    /* Attente du redimensionnement des images par les threads. */
    printf("Resizing images... ");
    while(*(shared.nb_images_done) < nbImages)
    {
        printf("\rResizing images... (%zu / %zu)", *(shared.nb_images_done) + 1, nbImages);
        fflush(stdout);
    }

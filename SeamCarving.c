...
void* thread_modifyImageWidth(void* arg)
{
    ...
    else if(input->k < 0) output = reduceImageWidth(input->image, (size_t)(-input->k));
    else output = increaseImageWidth(input->image, (size_t)(input->k));

    /* Modification de la valeur de "*(input->shared.nb_images_done)". */
    pthread_mutex_lock(input->shared.mutex);
    *(input->shared.nb_images_done) += 1;
    pthread_mutex_unlock(input->shared.mutex);

 ...
 *      img        An ordered list of images (in PNM format) representing the
 *                 comic boxes
 \* ------------------------------------------------------------------------- */
...
int main(int argc, char* argv[])
{
    ...
    comicWidth = (size_t)atoi(argv[1]);
    comicBorder = (size_t)atoi(argv[2]);
    /* --- Comic building --- */
    PNMImage* output = showEnergies(images[1]);
    writePNM(argv[3], output);
    freePNM(output);
